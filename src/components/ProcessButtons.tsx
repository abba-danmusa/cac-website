...
const ServiceButton: React.FC<ServiceButtonProps> = ({ icon, title, to, img }) => {
  return (
    <Link href={to} className="gradient-hover items-center group flex gap-[10px] lg:w-[544px]  sm:w-auto h-[77px] md:h-[104px] border-2 rounded-lg p-[10px] border-[#4D6C4A] transition duration-300">
      ...
      ? <h3 className={`"text-black font-sans ${title === "Post Incorporation Services" ? "block lg:hidden" : 'hidden'}  font-normal leading-5 md:leading-10 text-[18px] md:text-[34px] group-hover:text-primary"`}>post Inc. services</h3> 
      : null}
      <h3 className={`"text-black font-sans ${title === "Post Incorporation Services" ? "hidden lg:block" : 'block'}  font-normal leading-5 md:leading-10 text-[16px] md:text-[34px] group-hover:text-primary"`}>{title}</h3>
    </Link>
  );
};
